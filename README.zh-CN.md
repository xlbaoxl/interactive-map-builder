...
点击 **分享当前视图 → 下载当前视图 HTML**，即可得到一份独立地图。接收者打开后，会恢复
当前中心与缩放、搜索、筛选、排序、选中对象、图层显隐及面板状态。**导出视图 JSON** 只保存
查看状态；在相同地图和数据版本中点击 **导入视图 JSON** 即可恢复。它与浏览器本地的命名视角
独立，文件改名或移动不影响打开。

HTML 包含**全部内嵌对象**，包括隐藏和被筛除的记录；筛选不等于脱敏，请仅分享给有权接收的人。
在线底图仍需要网络。浏览器导出的副本与原始构建清单、重建包分开管理，请保留原始 `dist` 不变。
完整边界见[可移交视图状态说明](references/view-state.md)。
...
基础安装支持矢量／CSV 输入、现有地图样式及静态输出。Excel 读取依赖按需安装；在 Skill 根目录
选择与输入对应的方式：

```bash
python -m pip install .           # 矢量／CSV 输入
python -m pip install ".[excel]" # 增加 .xlsx 与 .xls 读取
python -m pip install ".[all]"   # 全部受支持的读取器
```

已有环境中安装的读取器会继续保留；缺少 Excel 读取器时，检查和读取命令都会给出安装提示。
Matplotlib 仍是声明依赖，以保留插值分级配色的原有颜色语义；普通 HTML 构建按需加载代码，
无需启动静态绘图后端或 Excel 读取器。版本／帮助和更新检查也会延迟加载 GIS 运行环境。
...
Agent 会从成果要求中确定模板：独立开关图层对应 `multilayer`，明确主清单对应 `map-list`。
需求明确时，检查后直接构建，多图层任务同样适用。只对坐标系缺失、分类含义不明、主图层尚未
确定、数据公开权限等真正的阻塞问题集中确认。标题、布局和背景透明度等可调整的展示选择先用
明确展示的默认值，在第一版验证后继续打磨。
